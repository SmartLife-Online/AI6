...
Jeder Run erhält genau einen Worktree unterhalb von `<managed-root>/projects/<projekt>/worktrees/<run>` und genau einen Branch `refs/heads/ai6/runs/<projekt>/<run>`, dessen geschlossene ASCII-Grammatik ausschließlich aus Projektkennung und Run-ID gebildet wird und einen von den verwalteten Control-Refs getrennten Namensraum belegt. Beide entstehen genau auf dem unveränderlichen `initial_run_base_sha`; das aufgelöste Branch-Objekt wird danach gegen genau diesen Anker geprüft. Ein späterer `run_base_sha` ist die aktuelle Candidate-Basis und wird nie als Worktree-Ahn verwendet. Die SQLite-Guards halten Branch und Worktree-Pfad nach dem Setzen unveränderlich. Eine Checkpointbindung darf ausschließlich über eine neue, lückenlos nummerierte `run_checkpoints`-Revision fortgeschrieben werden; der Vorgänger bleibt auditierbar, und ein bereits abgelöster Commit kann nicht wieder zum aktuellen Checkpoint werden.

Lokale Checkpoints werden nie gepusht. Sie entstehen mit fester, hostunabhängiger Commit-Identität und festem Zeitstempel und binden Commit-OID, Tree-OID sowie einen über `CanonicalJson` domänenseparierten Diff-Hash am Run; der Hash bindet die tatsächlichen kanonischen Diffdaten mit vollständigen Objektnamen, während ausschließlich das zentrale Redaction-Ergebnis persistiert oder angezeigt wird. Ein unveränderter wirksamer Stand verwendet idempotent denselben Checkpoint; eine Baum-, Scope- oder Vertragsänderung erzeugt eine neue Revision. Diff-, Tree- und Blob-Zugriffe eines Runs laufen lesend über dieselbe gehärtete Naht.
...
`FakeAgentAdapter` implementiert denselben `AgentAdapter`-Vertrag wie spätere reale Provideradapter. `result()` erzeugt ohne Zeitabhängigkeit oder Netzwerkzugriff byteidentisch die neun Szenarien Erfolg, No-change mit leerem Diff, No-change mit unzulässig nicht leerem Diff, Frage, Findings, ungültiges JSON, Providerfehler, Securityfindings und als Steueranweisung formulierte untrusted Evidenz; der Implementierungsturn `turn()` führt dasselbe Dokument über `ControlProcessRunner` und die Agent-Prozesspolicy in der exportierten Tree-Sicht aus. Für Reviewtests kann die Szenariofolge je stabiler Slot-UUID gebunden werden, ohne den Adaptervertrag oder einen Produktionsschalter zu erweitern. Die technische Nichterreichbarkeit von `.git`, Managed-Worktree und Credentials stützt sich dort auf PHP-`open_basedir` und ist bis zu den realen Adaptern (`AI6-033`, `AI6-034`) kein Nachweis für ein Provider-CLI. Das Szenario ist Zustand des Fakes und kein Feld des Turnkontexts. Seine Bytes durchlaufen unverändert denselben `AgentResultValidator` und sind als Golden-Vektoren bytegenau gebunden. Ein Implementierungsergebnis enthält zusätzlich die geschlossenen Felder `decisions`, `changed_paths`, `open_manual_gates` und `implementation_summary`.
...
Vor dem Qualitätsreview trifft ausschließlich `RunOrchestrator::reviewReadiness()` die Bereitschaftsentscheidung. Sie gleicht tatsächliche Pfade mit dem gebundenen wirksamen Scope ab, verlangt für jedes `before_review`-Profil ein wirklich ausgeführtes erfolgreiches Ergebnis des aktuellen Evidenzstands und der finalen Baum-Bindung, liest offene `MG-`-/`EXT-`-Gates, prüft Ticketvertrag und Control-Basis auf Drift und vergleicht den tatsächlichen kanonischen Diff mit dem aktuellen Checkpoint. Blockadegründe werden vollständig und in dieser Reihenfolge persistiert. Offene Gates verhindern den Qualitätsreview nicht, bleiben in `run_gates` aber ausdrücklich für Candidate, finalen Commit und Push blockierend. Autorisierte Evidenz bindet Ticketvertrag und Checkpoint; eine Scope-, Vertrags- oder Checkpointänderung öffnet das Gate erneut, ohne den bisherigen Evidenzverweis zu löschen. Die Timeline zeigt ausschließlich diese gespeicherten, redigierten Zustände und startet weder Git noch einen Prozess.

## Serielle Reviewrunde und unveränderliche Reviewergebnisse

Der registrierte Schritt `review` folgt auf die gebundene Reviewbereitschaft und bleibt auf demselben idempotenten Jobweg mit Lease und Intent. `RunOrchestrator` materialisiert die erforderlichen `quality_review`-Slots ausschließlich in der Reihenfolge des am Run gespeicherten Approval-Snapshots in `run_agents`; aktuelle Serverdefaults und Providertext können die Menge weder ergänzen noch ersetzen. Jeder Slot bindet Rolle, Providerprofil, Modell, Effort und Promptprofil. Seine Session ist global eindeutig, wird niemals zwischen Slots oder Runs geteilt und bei einer autorisierten Rückfrage nur für denselben Slot fortgesetzt.

Unmittelbar vor jedem Slot prüft `ReviewCheckpointVerifier` erneut den sauberen Worktree, den Head der gebundenen Run-Branch, Commit- und Tree-OID sowie den kanonischen Diff-Hash. Erst danach erzeugt der vorhandene `IsolatedTreeExporter` einen wegwerfbaren, gitmetadatenfreien und read-only Export. `CheckTreeBinding` bindet Pfad, Typ, Ausführbarkeit und Inhaltsdigest; alle Slots einer Runde müssen denselben Workspacehash sehen. Jeder Aufruf erhält unter einem ausschließlich für diese Invocation erzeugten Elternverzeichnis zusätzlich ein frisches versiegeltes `ExecutionHome`; der Prozess darf nur dieses Verzeichnis mit eigenem Baum und eigener IO-Ablage erreichen. Das Home trägt exakt die freigegebenen Instruction-Bytes und das Runtimeprofil dieses Slots und wird nach dem Turn zusammen mit Export und Invocation-Verzeichnis entfernt. Managed-Worktree, aufgelöster Git-Dir, Common-Dir, Refs, fremde Homes und andere Slots bleiben aus dem Turn unerreichbar.

Globale Config-, Scope-, Prompt-, Instruction-, Runtime-, Agenten- und Security-Approvalhashes sind für die Runde identisch. Daneben werden der konkrete Prompt-, Provider-Instruction- und Runtimehash je Slot gegen dessen Snapshot geprüft; unterschiedliche Profile werden nicht künstlich auf denselben Hash vereinheitlicht. Das initiale Kontextpaket enthält nur Rolle, Slot, Versuch und die gebundenen Kriterien, aber keine Ergebnisse, Findings oder Rohausgaben eines anderen Reviewers.

Jeder Aufruf hinterlässt genau eine append-only Zeile in `review_results`. Die Datenbank bindet Run, Runde, Slot, Versuch, Session, Checkpoint, Diff, globale und slotbezogene Hashes, Workspacehash, Ausgang, benannten Fehler, validierten Status und den optionalen redigierten `provider_raw`-Artefaktverweis. Trigger verhindern Updates und Löschungen; ein partieller Unique-Index erlaubt je Runde und Slot höchstens ein gültiges Ergebnis. Providerfehler, ungültiges JSON, Binding-, Checkpoint-, Workspace- und Human-Request-Fehler bleiben unterscheidbar und werden niemals in ein gültiges Ergebnis umgedeutet. Eine Reviewerfrage nutzt ausschließlich den bestehenden slot- und schrittgebundenen `HumanRequestService`; `provider_error`, `invalid_json` oder ein Reviewlimit werden in diesem Schritt nicht als neue Wartestatus registriert. Providerprosa bleibt redigierte Evidenz ohne Mutationsautorität über Scope, Ticket, Run, Policy oder Slotauswahl.

## Checkprofile und credentialfreier Checker

Ein Projektcheck ist ausschließlich ein serverseitig definiertes Profil. `config/ai6.php` ist die einzige vertrauenswürdige Quelle und legt je Profil Programm, Argumentliste, erlaubte Phasen, Arbeitsverzeichnis, die als Erfolg geltenden Exitcodes sowie die Metadaten zu Seiteneffekt, Netzzugriff und Mutation fest. `CheckProfileRegistry` ist die eine Auflösung dieser Namen; `CheckProfileAllowlist` leitet die für eine untrusted Projektkonfiguration zulässigen Namen aus genau dieser Registry ab, sodass Validierung und Ausführung nicht auseinanderlaufen können. Eine zweite Namensliste existiert nicht mehr.

Ein Projekt wählt in `.ai6/config.yaml` einen Namen und sonst nichts. Ein Shellstring, ein Shellinterpreter als Programm, ein Inlinecode-Flag, ein zusätzliches Argument, ein Programm außerhalb der Checker-Prozesspolicy und ein Profil ohne vollständige Metadaten enden als benannter, wertfreier Konfigurationsfehler und werden nie ausgeführt.

Ausgeliefert sind drei Profile, abgeleitet gegen die tatsächlich im Image vorhandenen Programme PHP und Git:

| Profil | Kommando | Phase | Erfolgs-Exitcodes |
|---|---|---|---:|
| `php-targeted` | `php artisan test --compact` | `before_review` | `0` |
| `php-all` | `php artisan test` | `final` | `0` |
| `git-diff-check` | `git --no-pager diff --check --no-index -- baseline tree` | `before_review`, `final` | `0`, `1` |

Die exportierte Tree-Sicht trägt nach `GIT-010` keine Gitmetadaten, deshalb kann `git diff --check` nicht im Baum selbst laufen: Nur die repositoryfreie `--no-index`-Form gegen ein stets leeres `baseline`-Verzeichnis funktioniert, und ihr sauberer Exitcode ist `1`, nicht `0` — `3` meldet Whitespace-Fehler oder Konfliktmarker. Das Arbeitsverzeichnis ist deshalb je Profil entweder `tree` (der geprüfte Baum) oder `batch` (dessen Elternverzeichnis mit `baseline` und `tree`).

Weil `baseline` leer ist, prüft `git-diff-check` bewusst den **gesamten** exportierten Baum und nicht die Änderung dieses Runs: Jede vorbestehende Whitespace-Verletzung im Repository lässt es fehlschlagen. Als ausdrücklich gewähltes Hygieneprofil ist das brauchbar, als verpflichtendes Finalgate wäre es für jedes Projekt mit Altbestand unpassierbar. Die Serverdefaults nennen es deshalb nicht; `server_defaults.checks.final` enthält nur `php-all`. Die Bindung an die Änderungsdifferenz verlangt den Basisbaum neben dem aktuellen und gehört in das Ticket, das den Checkpoint-Diff besitzt.

`CheckRunner` löst den Profilnamen auf und prüft die Phase. Im strikten Normalweg bindet der Worker vor jeder Wirkung eine stabile Ausführungs-ID und Frist im Schritt-Intent, exportiert genau eine symlinkfreie Quelle samt leerem `baseline` in die Checker-Eingabe und schreibt einen geschlossenen Auftrag ohne Programm, Argumente oder Metadaten. Die Checker-Schleife claimt den Umschlag atomar, löst das Profil erneut aus der Registry auf, kopiert Quelle und `baseline` in das checkerprivate Workspace und startet genau einen Prozess über `ControlProcessRunner::start()` mit Checkerpolicy und Argumentliste. Ein zweiter Prozess- oder Mailboxpfad entsteht nicht. Die Environment-Allowlist wird zur Laufzeit aus der Checkerpolicy gelesen statt dupliziert, sodass Policy und Request nicht auseinanderdriften können.

**Ein Checkprozess läuft im Normalweg ausschließlich in der isolierten Checkerrolle.** Solange die Kontrolle `AI6_SECURITY_REQUIRE_CHECKER_NETWORK_ISOLATION` aktiv ist — im Profil `strict` immer, und dort nicht abschaltbar — führt der Worker nur Staging, Auftrag und gebundenes Polling aus. Der Checkerprozess erhält weder Provider-, Git-, SMTP- oder Datenbankcredentials noch eine Variable außerhalb der Policy-Allowlist. Das Checker-Containernetz bleibt mit `network_mode: none` vollständig abgeschaltet.

Der Grund ist nicht formal: Ein Checkprofil führt den Code des verwalteten Projekts aus, also untrusted Repositoryinhalt (`SEC-007`). Nur der Checkercontainer trägt die Zusagen, die das erträglich machen — `network_mode: none`, keine Provider-, Git-, SMTP- oder Datenbankcredentials, kein Managed-Clone-Volume, read-only Root und der vollständige `ProcessIsolationVerifier`. Der Worker dagegen hat normales Netz und mountet `ai6_managed` samt Deploy-Keys; dort ausgeführter Projektcode könnte beides erreichen. `CheckContainmentBoundary` prüft Arbeitsverzeichnis und Ergebnisverzeichnisse und ist genau deshalb **keine** ausreichende Ersatzgrenze.

Die Ausführung außerhalb der Checkerrolle ist damit eine ausdrückliche, sichtbare Reduktion: Sie verlangt ein nicht-striktes Securityprofil, das abgeschaltete Maß und ein bestätigtes `acknowledge_reduced_mode`, geht in den Policyhash ein und erscheint im Doctor-Banner. Für Entwicklung und Tests ist das der vorgesehene Weg; für den Betrieb ist es keiner.

Der feste Namespacepfad verwendet das aus dem gepinnten `util-linux` vorhandene `/usr/bin/unshare` und den im Image read-only inventarisierten `checker-process-wrapper.sh`. Er erzeugt vor dem Profilprogramm eine eigene Mount- und PID-Sicht und maskiert Checker-Eingabe, -Ausgabe und beide Heartbeatkanäle. Die Argumentliste, Limits, Prozessgruppe und der kontrollierte Abbruch bleiben beim zentralen `ControlProcessRunner`. Jede fehlende Rollen-, Mount-, Root-, Netz-, Gitmetadaten-, Symlink-, Werkzeug- oder Namespacezusage weist den Start benannt ab.

Der Checker bindet dafür ausschließlich `docker/checker-seccomp-moby-29.6.1.json`. Die Datei basiert auf der offiziellen Moby-Defaultpolicy des Tags `docker-v29.6.1` und ergänzt genau `unshare`, `mount` und `umount2` für die innere User-/Mount-Namespacegrenze. Sie ersetzt weder die capabilityfreie Containeridentität noch `setpriv --bounding-set=-all`; kein anderer Dienst lädt diese Policy, und `seccomp=unconfined` sowie `cap_add` bleiben verboten. Ein Docker-Engine-Wechsel verlangt eine erneute Ableitung und einen realen Checker-Probelauf.

Während des Prozesses schreibt die eine Checker-Schleife sowohl ihren lokalen Boot-Heartbeat als auch einen atomaren, ausführungs- und bootgebundenen Heartbeat in den Rückkanal. Worker-Polls verwenden weiterhin dieselbe Ausführungs- und Delivery-ID und werden beim Parken nicht auf das Job-Retrybudget angerechnet. Fristüberschreitung, veralteter Heartbeat oder ein neuer Checker-Boot gegenüber dem alten Ausführungsheartbeat sind terminale, nicht grüne Grenzen; ein spätes Ergebnis wird nicht mehr persistiert. Nach Ergebnis oder Grenze werden Staging, privates Workspace, Umschläge, Claimmarker, Prozessausgabe und Ausführungsheartbeat gebunden bereinigt. Ein unsicherer Rest blockiert statt grün fortzufahren.

Die Checker-Schleife veröffentlicht außerdem je Boot eine frische Selbstprüfung von Rolle, Mountoptionen, read-only Containerroot, Netzgrenze, Namespacewerkzeug und ausführbaren Profilprogrammen. `ai6:doctor` wertet diese Attestation sowie die workerseitig sichtbaren Ein-/Ausgabewurzeln aus; fehlende, alte oder unvollständige Evidenz ist `FEHLER`. Der bisherige synchrone Direktpfad bleibt ausschließlich für eine ausdrücklich bestätigte Reduktion aus nicht-striktem Profil, abgeschalteter Checker-Isolationsmaßnahme und `acknowledge_reduced_mode=true` erhalten.

Die auszuführende Profilmenge stammt ausschließlich aus dem an den Run gebundenen freigegebenen Konfigurations-Snapshot; der Inhalt des aktuellen Worktrees und eine spätere Änderung der Serverdefaults verändern sie nicht. Ein fehlender oder strukturell ungültiger Checkblock im Snapshot wird nie als „keine Checks nötig" gelesen: Der Schritt endet fail closed als `check_snapshot_invalid`. Nur eine gültig gebundene, tatsächlich leere Liste schließt den Schritt ohne Check ab.

Ein Ergebnis kennt genau vier unterscheidbare Zustände, die nie ineinander abgebildet werden:

| Zustand | Bedeutung |
|---|---|
| `succeeded` | Exitcode gehört zu den deklarierten Erfolgscodes |
| `failed` | Check ist inhaltlich fehlgeschlagen |
| `timed_out` | Laufzeitgrenze der Checkerpolicy erreicht |
| `tool_unavailable` | Programm nicht ausführbar, Start abgelehnt oder Exitcode `127` |

Jedes Ergebnis trägt zusätzlich einen benannten Grund, Exitcode, Laufzeit, die redigierte Ausgabe, die deklarierten Metadaten und die Baumbindung vor und nach dem Lauf. Vor und nach jedem Check wird der geprüfte Baum deterministisch über Pfad, Typ, Ausführbarkeit und Inhaltsdigest gebunden. Ein als nicht mutierend deklariertes Profil, das den Baum dennoch verändert, endet als `unexpected_tree_mutation` und nie als grünes Ergebnis; ein ausdrücklich mutierendes Profil hinterlässt seine Änderung sichtbar im Ergebnis. Weil der Check auf einem beschreibbaren Export läuft, erreicht keine dieser Änderungen den Reviewstand unbemerkt.

Jede Prozessausgabe passiert vor Persistenz und Anzeige denselben zentralen `Redactor` wie Read Models und Provideroutput. Das Checkmodul führt keine eigene Secret- oder Redaction-Musterliste; ungültiges UTF-8 wird verworfen statt ausgegeben.

Ergebnisse liegen in `check_results` mit einem deterministischen Schlüssel aus Run, Evidenzepoche, Phase, Profil und geprüftem Baum. Die Eindeutigkeit gilt partiell für die jeweils lebende Zeile (`superseded_at IS NULL`). Ein Epochenwechsel ist auch auf einem unveränderten Baum eine neue Ausführung; ein erfolgreiches Ergebnis aus einer älteren Epoche kann den aktuellen Run daher weder erfüllen noch blockieren. Der lebende Datensatz wird geprüft, **bevor** ein Prozess starten kann; eine doppelt zugestellte Schrittnachricht erzeugt daher weder einen zweiten Checkerprozess noch ein zweites Ergebnis. Die Mailbox-Delivery trägt zusätzlich den Zustellversuch, damit der Auftrag eines abgestürzten Versuchs den nächsten nicht blockiert. Trifft eine Delivery dennoch auf den Auftrag genau ihres eigenen Versuchs ohne zugehöriges Ergebnis, ist das der Absturz zwischen Auftrag und Ergebnis: Der Schritt endet benannt als `check_execution_orphaned`, statt still so zu tun, als sei der Check gelaufen.

Zeilen sind unveränderlich. Die einzige erlaubte Änderung ist das einmalige, einseitige Supersede eines lebenden, nicht erfolgreichen Ergebnisses — genau der Weg, den der Resolver `retry_unchanged_tree` geht: Er markiert den fehlgeschlagenen Vorgänger als abgelöst und gibt damit die Ausführung frei, sodass ein Retry auf dem unveränderten Baum wirklich ein zweiter Lauf ist und nicht mit sich selbst kollidiert. Der Vorgänger wird nie gelöscht; seine Evidenz bleibt lesbar.

Der Schritttyp `check` besitzt einen registrierten Handler auf dem bestehenden idempotenten Jobweg mit Lease, Retry und Intent und folgt dem abgeschlossenen `implement`-Schritt in der Phase `check`. Er führt die Phase `before_review` aus; die Phase `final` gehört in den Finalisierungsablauf (`AI6-027`). Ein fehlgeschlagener Check setzt `wait_reason=check_failure` und parkt den Schritt. Dieser Wartestatus ist mit genau einem Producer (`RunCheckStep`, der den Run parkt), seinen Resolvern (`retry_unchanged_tree`, `orchestrator_code_fix`) und einem Cancelpfad registriert; `CheckFailureResolver` führt Retry und Cancel aus, ändert den Runzustand aber ausschließlich über den `RunOrchestrator`.
