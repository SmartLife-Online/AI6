...
| `AI6_CONTROL_OPERATION_REFRESH_BASE_PATH` | `tickets` | kanonischer repositoryrelativer Basispfad für Einzelpfad-Refresh; ausschließlich Serverkonfiguration |
...
`ticket_refresh` ist eine typisierte asynchrone Control Operation für genau einen repositoryrelativen Pfad. Weil sie die exklusive Projektoperationssperre beansprucht, dürfen nur globale Administratoren und Projektoperatoren sie einreihen; Viewer und Approver bleiben auf den lesenden Zugriff beschränkt. Die Oberfläche liefert nur einen untrusted Kandidaten. Server und Worker kanonisieren ihn unabhängig und akzeptieren ihn ausschließlich unter `AI6_CONTROL_OPERATION_REFRESH_BASE_PATH`, standardmäßig `tickets`; weder Browserinput noch Repositoryinhalt setzt oder erweitert diesen Basispfad. Der Worker liest am gebundenen aktiven Control-Commit ausschließlich einen case-genauen regulären Git-Blob. Absolute Pfade, `..`, Backslashes, Steuerzeichen, Symlinks, Trees, Submodule und Pfade außerhalb des Basispfads werden ohne Read Model abgewiesen. Ein Blob mit ungültigem UTF-8 endet als benannter terminaler Konflikt statt als Retry. Eine Verzeichnisdurchsuchung findet nicht statt.

Jede Projektion bindet Projekt, Operation, relativen Pfad, Control-Commit, Blob-SHA, `control_generation` und Erzeugungszeit. Vor `AI6-007` bleibt `document_state` ausschließlich `unparsed`; `validation_profile` und `ticket_contract_sha256` sind `null`. Der zentrale `Redactor` erzeugt den gespeicherten Inhalt sowie Typ, Feld/Span, Marker, Fingerprintversion, Key-ID und projektgebundenen HMAC-Fingerprint je Treffer. Entfernte Klartexte und unkeyed Digests werden nicht gespeichert. `source_blockers` und `approval_editor_eligible=false` sperren `unparsed` und jede inhaltlich redigierte Projektion fail closed als Approval- oder Editorquelle. Ausschließlich die zentrale `TicketReadModelUsePolicy` entscheidet, ob eine Projektion als Approval- oder Editorquelle verwendet werden darf.
...
- [Implementierungsplan V1.7.0](docs/AI6_IMPLEMENTATION_PLAN.md) — normative Quelle für Anforderungen, Architektur, Meilensteine und Ticket-Blueprints.
