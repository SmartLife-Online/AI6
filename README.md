...
`FakeAgentAdapter` implementiert denselben `AgentAdapter`-Vertrag wie spätere reale Provideradapter und erzeugt ohne Prozess, Zeitabhängigkeit oder Netzwerkzugriff byteidentisch die acht Szenarien Erfolg, No-change mit leerem Diff, No-change mit unzulässig nicht leerem Diff, Frage, Findings, ungültiges JSON, Providerfehler und Securityfindings. Das Szenario ist Zustand des Fakes und kein Feld des Turnkontexts. Seine Bytes durchlaufen unverändert denselben `AgentResultValidator` und sind als Golden-Vektoren bytegenau gebunden.
