...
`CheckRunner` löst den Profilnamen auf, prüft die Phase, schreibt genau einen Auftrag in die vorhandene Checker-Mailbox und startet den Prozess über `ControlProcessRunner` mit der vorhandenen Checkerpolicy und Argumentliste. Ein zweiter Prozess- oder Mailboxpfad entsteht nicht. Die Environment-Allowlist wird zur Laufzeit aus der Checkerpolicy gelesen statt dupliziert, sodass Policy und Request nicht auseinanderdriften können. Der Checkerprozess erhält weder Provider-, Git- noch SMTP-Credentials, keine Primärdatenbank und keine Variable außerhalb dieser Allowlist. Netzzugriff ist Standard aus: Der Checkercontainer läuft unter `network_mode: none`, die Isolationsprüfung fordert in der Checkerrolle das Fehlen jeder Netzwerkschnittstelle, und ein Profil, das ausdrücklich Netzbedarf deklariert, wird fail closed als `check_profile_network_unavailable` abgewiesen, statt still ohne Netz zu laufen.

**Ein Check läuft ausschließlich in der isolierten Checkerrolle.** Solange die Kontrolle `AI6_SECURITY_REQUIRE_CHECKER_NETWORK_ISOLATION` aktiv ist — im Profil `strict` immer, und dort nicht abschaltbar — verweigert `CheckRunner` die Ausführung in jeder anderen Rolle, bevor ein Prozess startet und bevor ein Ergebnis existiert. Der Schritt endet benannt als `check_execution_role_required`, niemals grün.
...
Der produktive Vollzug — Worker staged den Baum in das Checker-Volume und schreibt den Auftrag, der Checker konsumiert ihn in seiner Rolle und publiziert das Ergebnis über die vorhandene Ergebnisnaht zurück — ist noch nicht gebaut: `ai6:execution-mailbox checker` führt bislang nur den Heartbeat-Loop aus. Bis dahin läuft in einer strikten Instanz kein Check, und das ist sichtbar, statt still unsicher zu sein.
