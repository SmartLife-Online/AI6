...
final class PromptHelpBrowserSmokeTest extends AuthFeatureTestCase
{
    ...
    public function test_clipboard_success_fallback_selection_and_mobile_width(): void
    {
        ...
        if (getenv('AI6_BROWSER_SMOKE') !== '1'
            ...
            self::markTestSkipped(
                'The prompt-help browser smoke test requires AI6_BROWSER_SMOKE=1 and an existing AI6_BROWSER_SMOKE_CHROMEDRIVER_BINARY.',
            );
        }
    ...
    {
        $path = sys_get_temp_dir().'/ai6-prompt-help-smoke-'.bin2hex(random_bytes(8)).'.sqlite';
    ...
    private function setViewport(int $width, int $height, bool $mobile): void
    {
        $response = $this->driverRequest('POST', '/session/'.$this->sessionId.'/goog/cdp/execute', [
        ...
        $this->driverRequest('POST', '/session/'.$this->sessionId.'/window/rect', [
            'x' => 0,
            'y' => 0,
            'width' => $width,
            'height' => $height,
        ...
        $innerWidth = $this->execute('return window.innerWidth;');
        self::assertIsNumeric($innerWidth);
        self::assertSame(
            $width,
            (int) $innerWidth,
    ...
    private function assertNoHorizontalScrolling(string $viewName, int $width, int $height): void
    {
