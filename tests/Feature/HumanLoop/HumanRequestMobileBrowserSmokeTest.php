...
final class HumanRequestMobileBrowserSmokeTest extends TicketUiTestCase
{
    ...
    {
        $path = sys_get_temp_dir().'/ai6-hr-browser-smoke-'.bin2hex(random_bytes(8)).'.sqlite';
    ...
    private function createBrowserSession(): void
    {
        $chromeOptions = [
            'args' => [
                '--headless=new',
     ...
     */
    private function driverRequest(string $method, string $path, array|\stdClass|null $body = null, bool $allowError = false): array
    {
    ...
    private function webDriverHttpRequest(string $method, string $path, array|\stdClass|null $body = null): string|false
    {
        ...
        $socket = @stream_socket_client('tcp://'.$host.':'.$port, $errorCode, $errorMessage, 30, STREAM_CLIENT_CONNECT);
        if ($socket === false) {
            return false;
        }
