...
Die M5-Tickets `AI6-027` bis `AI6-032` wurden am 30. August 2026 gegen den integrierten Stand `cf4183a` erzeugt. `AI6-027` wurde vollständig gegen den damaligen realen Repositoryzustand abgeleitet. `AI6-028`, `AI6-029`, `AI6-030`, `AI6-031` und `AI6-032` wurden auf ausdrückliche menschliche Anordnung vor der Umsetzung ihrer damals offenen Abhängigkeiten erzeugt und waren damit bei ihrer Erzeugung **ahead-derived** im Sinne von Plan §13.6: `AI6-028` fehlte `AI6-027`, `AI6-029` fehlten `AI6-027` und `AI6-028`, `AI6-030` fehlte `AI6-029`, `AI6-031` fehlten `AI6-029` und `AI6-030`, und `AI6-032` fehlten `AI6-029`, `AI6-030` und `AI6-031`. Jede fehlende Voraussetzung war beziehungsweise ist ein bereits in Plan §15 definierter Blueprint. `AI6-032` wurde am 4. September 2026 auf ausdrücklichen menschlichen Auftrag gegen den integrierten Stand `840de1afc696cb048995baa66a92ab76776fd81d` nach Umsetzung und menschlicher Abnahme aller fünf `depends_on`-Tickets rebased. Das Rebase-Gate ist geschlossen; `status: todo` und vorhandene AC-/TC-/MG-/EXT-IDs blieben unverändert. Rebase und Statusänderung bleiben getrennte menschliche Entscheidungen.

`AI6-027`, `AI6-029` und `AI6-031` nehmen in ihrem `## Context` jeweils ausdrücklich die Ausnahme aus Plan §13.2 in Anspruch und begründen sie: Candidate und Candidate-Gate, Abschlusskette aus Commit, Push, Statussynchronisation und Sperrfreigabe sowie Beobachtung samt Retentiondurchsetzung sind jeweils nicht unabhängig lieferbar, weil ein Wartegrund ohne Resolver den Bootstrap fehlschlagen lässt und die übrigen Teile ohne einander keinen prüfbaren Zustand erzeugen. `AI6-032` deklariert als einziges dieser sechs Tickets ein manuelles Gate: `MG-01` verlangt die menschliche Abnahme des Release-Gates im realen Compose-Stack unter Linux, weil ein erheblicher Teil der Prozess-, Git- und Checkernachweise POSIX-Semantik beweist und sich außerhalb dieser Laufzeit selbst überspringt.

`AI6-028` wurde am 31. August 2026 auf ausdrücklichen menschlichen Auftrag gegen den integrierten Stand `8175a64f32677972ef5060eb5b224312e288ee21` rebased. Das Rebase-Gate ist geschlossen; `files`, Scope-Marker und die konkreten Candidate-, Finalisierungs-, Reviewresultat-, Security-, HumanLoop-, Wartestatus- und Testnähte wurden gegen den realen Stand neu abgeleitet. Status und vorhandene AC-/TC-/MG-/EXT-IDs blieben unverändert.

`AI6-029` wurde am 1. September 2026 auf ausdrücklichen menschlichen Auftrag gegen den integrierten Stand `f997f38204228d7cee209f0451352fdcc76bdff9` nach dem umgesetzten und menschlich abgenommenen `AI6-028` rebased. Verifiziert wurden die realen Candidate-, Security-, Schrittplanungs-, Git-, Statussaga-, Recorded-Scope-, HumanLoop-, Wartestatus- und Testnähte. Das Rebase-Gate ist geschlossen; Status und vorhandene AC-/TC-/MG-/EXT-IDs blieben unverändert.

`AI6-030` wurde am 1. September 2026 auf ausdrücklichen menschlichen Auftrag gegen den integrierten Stand `7c94bff` nach dem umgesetzten und menschlich abgenommenen `AI6-029` rebased. Verifiziert wurden die realen Publishabschluss-, Statussaga-, Run-/Sperrfreigabe-, Cleanup-, Approval-, Eligibility-, Dependency-, Generation-, Routen- und Testnähte sowie alle `files` und Scope-Marker. Das Rebase-Gate ist geschlossen; der Status `todo` und vorhandene AC-/TC-/MG-/EXT-IDs blieben unverändert.
