...
├── artisan, app/, bootstrap/ ...    integrated and human-accepted AI6-001 … AI6-020 and AI6-044
...
- Deriving any ticket ahead of its dependencies remains a human-ordered exception under §8.1. Every ahead-derived ticket so far — `AI6-012` … `AI6-017` and `AI6-044` — has been rebased against its real dependencies; `tickets/README.md` records which ones and against what.
- A blueprint without a detail ticket is not a work order. `AI6-045` exists only in plan §15; generating its ticket is a separate human instruction under §8.
