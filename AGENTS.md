...
├── artisan, app/, bootstrap/ ...    integrated and human-accepted AI6-001 … AI6-032, AI6-039, AI6-040, AI6-043, AI6-044 and AI6-045
├── Dockerfile, docker-compose.yml    single image, process roles, worker volume, proxy net, isolated agent and checker services
├── docker/, deploy/                  role dispatcher with init provisioning, checker seccomp profile; Caddy reverse-proxy profile
├── scripts/                          ticket-manifest generator with drift check, PHPStan memory limit, local run and verification helpers
├── tests/                            PHPUnit suite; part of it requires the Linux runtime and skips elsewhere
├── docs/
│   ├── AI6_IMPLEMENTATION_PLAN.md   normative source, revision V1.7.5, German
...
│   ├── AI6-032_FOLGEAUFTRAG_IMPLEMENTIERUNGSISOLATION.md   decision request that plan revision V1.7.5 answered with blueprint AI6-046
│   ├── AI6_M6_ADAPTER_REVIEW.md     decision request from the M6 adapter-ticket review, answered by plan revision V1.7.5 with AI6-046 and AI6-047
│   └── AI6-004_VERIFICATION.md, AI6-005B_MG-01_…, AI6-006C_MG-01/02_…, AI6-008_MG-01_…, AI6-032_MG-01_…, AI6-044_MG-01_…, AI6-045_MG-01_…   human gate protocols
└── tickets/
    ├── README.md                    backlog overview; a view, never a status source
    ├── AI6-001.md … AI6-032.md, AI6-039.md, AI6-040.md, AI6-043.md, AI6-044.md, AI6-045.md   all status done
    ├── AI6-033.md, AI6-034.md, AI6-041.md, AI6-042.md   the M6 adapter drafts, status todo, ahead-derived since plan revision V1.7.5
    ├── AI6-035.md                   provider onboarding, status todo, ahead-derived
    └── AI6-046.md, AI6-047.md       the shared M6 prerequisites, status todo; AI6-046 fully derived, AI6-047 ahead-derived
```

`AI6-001` through `AI6-032`, `AI6-039`, `AI6-040`, `AI6-043`, `AI6-044` and `AI6-045` are integrated and human-accepted; those forty-three ticket files carry `status: done`, and the current frontier commit is `2f89d8e` (`AI6-032`). Each `done` status is an explicit human decision recorded in Git, and several were recorded separately from their implementation candidate — for example `16e5e53` for `AI6-007`, `d6839be` for `AI6-008` at candidate `028f799`, and `6b615e4` for `AI6-014`. Milestones M0 through M5 are therefore complete in code; M6 has not started.

What that means in code, module by module. **Auth**: users, project roles, passkey/TOTP/recovery enrollment, step-up and the e-mail barrier. **Git**: the hardened runner, control-operation core with operation lease and effect lock, deploy-key provisioning, managed clone, clone/fetch, control-branch change and invalidation generation, run branch/worktree/checkpoint/diff, the isolated git-metadata-free tree export, the validated patch import, review-subject normalization, publish candidate with provenance preflight, ticket mutation and the final commit/push path. **Projects**: project registration, the closed project configuration schema with approved snapshots, and blob-bound ticket read models. **Tickets**: the V1 parser/validator projection with the split editor/approval availability decisions in `TicketReadModelUsePolicy`. **Prompts**: the one central catalog and canonical renderer, review prompt profiles, plus the manual prompt helper of `AI6-044` at `/prompts/help`. **Agents**: agent/capability/runtime profiles, instruction snapshot resolution, the `ai6.agent.v1`/`ai6.quality-review.v1`/`ai6.finding-verification.v1` validators, `FakeAgentAdapter`, execution home and credential projection. **Checks**: server-defined check profiles, the check runner with deterministic tree binding, and check execution in the isolated credential-free checker role behind the execution mailbox. **Runs**: the run state machine and project lock, approval snapshots and queue, the execution-job path with lease/retry/intent, preflight, the implementation turn with diff import and run artifacts, adaptive scope with quarantine, contract amendments, the check step, pre-review verification, review-only runs with the report-only completion saga, the fix turn, finalization and publish completion, the project queue with auto-start, retention sweep, cancellation and the recorded scope. **Reviews**: review rounds on read-only workspaces, the review result store, findings with dispositions and effective state, finding verification rounds with verifier slot selection, review stall fingerprints and the fake-bound security review step. **HumanLoop**: human requests, e-mail, attention inbox, resume, scope approval, gate evidence and the publish/report-only/security-gate bindings. **Shared**: typed config, the central redaction boundary, the process/mailbox/isolation seams, the doctor, the security policy and HTTP/CSP hardening.

The gate-protocol documents in `docs/` keep distinct documentary states, and only one of them is closed: `AI6-008/MG-01` is filled, signed and passed for candidate `028f799`. The `AI6-006C/MG-01`, `AI6-006C/MG-02`, `AI6-032/MG-01`, `AI6-044/MG-01` and `AI6-045/MG-01` forms are result-free templates, and the `AI6-005B/MG-01` decision protocol is a filled draft whose signature is explicitly outstanding. Note the resulting tension: `AI6-032`, `AI6-044` and `AI6-045` carry `status: done` while their declared `MG-01` gates are still open. That is a human decision to resolve, not something to reconcile by editing either side. Never change a status or gate result; only the signed protocol at its bound candidate and the separate recorded human decision are acceptance evidence (§10). The FakeAgent release gate of `AI6-032` (`ai6:release-gate`) still declares the coverage gaps `AC-02` and `AC-04` and exits non-zero on purpose.

What does **not** exist yet as product runtime: provider CLI adapters for Codex, Grok, Copilot and Claude — `FakeAgentAdapter` is the only implementation of `AgentAdapter` — a persistent provider credential store, and `.ai6/` as a real managed-project directory. Two seams the M6 adapters need are also missing and were given their own blueprints by plan revision V1.7.5: the implementation turn passes a raw export to the adapter instead of a sealed execution home with a writable change output (`AI6-046`), and the agent-role execution mailbox consumes no turn requests, so a real provider turn would today start inside the worker (`AI6-047`).

Of the 53 planned tickets, 50 exist as detail-ticket files; only `AI6-036`, `AI6-037` and `AI6-038` are blueprints only. Plan revisions V1.6.2, V1.6.5, V1.6.7 and V1.6.10 progressively split the former `AI6-005` and `AI6-006`, so M0 ends with `AI6-005A`/`AI6-005B` and the M1 chain is `AI6-006A` … `AI6-006F`; the IDs `AI6-005` and `AI6-006` no longer exist. Plan revision V1.7.0 integrated `docs/AI6_erweiterungsauftrag_modellrouting_multi_review_pipeline_v2.md` — review-only mode, source-dependent advisory finding verification, review prompt profiles and the first real provider tier Codex/Grok/Copilot — and appended the blueprints `AI6-039` … `AI6-043`; `AI6-034` stays the unchanged Claude blueprint and no longer blocks `AI6-035`. Plan revision V1.7.1 added `AGT-011`, `UI-007` and blueprint `AI6-044` for the manual, project-independent prompt helper. Revision V1.7.2 unified the unredacted ticket projection on `redaction_state=clear`. Revision V1.7.3 recast the ticket file scope: `files` is the ticket's best guess and not a closed list (`TKT-007`), a path that is neither auto-allowed nor sensible is taken automatically under the unchanged `max_added_scope_paths` and documented instead of blocking the run, the trusted project setting `scope.unlisted_paths` keeps the strict variant available, and `TKT-012` has AI6 write the resulting effective scope back into the ticket file as `## Recorded Scope` — excluded from `ticket_contract_sha256`, never written by an agent. Revision V1.7.4 split `AI6-021` under §13.7 and gave the role-correct check execution the never-used ID `AI6-045`; with it integrated, a check executes in the isolated checker role behind the execution mailbox, and running one outside that role requires a non-strict profile plus a confirmed reduced mode, stays visible in the policy hash and remains impossible under `strict`. Revision V1.7.5 (6 September 2026) answered the M6 adapter review: `AI6-046` gives implementation and fix turns the same server-bound execution home as review turns plus a writable change output the worker imports from, and closes the `AI6-032/AC-04` gap afterwards; `AI6-047` hands a provider turn to the agent role over the existing mailbox after the `AI6-045` pattern, resolves the provider alias exactly once, and returns answer bytes, end state and usage values that the worker stores as provider-artifact metadata and maps to `invalid_json` or `provider_error`. `AI6-033` and `AI6-034` now depend on both, `AI6-041` and `AI6-042` on `AI6-047`; the backlog grew from 51 to 53 blueprints.
...
- Verify every path with `rg --files` or `ls` before naming it. Most contracts you will consume already exist in code, so read the real class before assuming a seam.
- Deriving any ticket ahead of its dependencies remains a human-ordered exception under §8.1. Right now `AI6-035` (waiting for the three V1 adapters), `AI6-047` (waiting for `AI6-046`) and the four adapter drafts `AI6-033`, `AI6-034`, `AI6-041`, `AI6-042` (waiting for `AI6-046`/`AI6-047` since revision V1.7.5) are ahead-derived: their rebase gates are open, they stay on `status: todo`, and they may be neither released nor claimed until rebased. `AI6-046` is fully derived against `2f89d8e` and is the next implementable ticket. `tickets/README.md` records which tickets were derived this way and against what.
- The seven M6 drafts are uncommitted; under plan §13.7 their `AC-`/`TC-`/`MG-` IDs stay draft-mutable until their first commit, and the adapter drafts record their old-to-new ID mapping in `## Notes`.
- A blueprint without a detail ticket is not a work order. `AI6-036`, `AI6-037` and `AI6-038` exist only in plan §15; generating one of their tickets is a separate human instruction under §8.
...
During implementation, review, and findings-fix iterations, run the tests bound to the changed behavior and to every contract the change touches: the new or modified tests, and the unchanged regression, inventory and architecture tests that pin those contracts — exact API inventories, the Compose and route contracts, the scaffold structure, the migration round trips. A complete regular-suite run is not required after every individual review step or finding fix; it remains the completion gate. New or changed error paths must still be exercised before the iteration is treated as verified. Before reporting an implementation ticket as done, run the applicable regular tests, `pint --test`, PHPStan, manifest and Composer checks, and `git diff --check` (plan §12.2). Run the external locked-install suite when dependency, lockfile, platform or installation behavior is in scope. Red is red — report the result with its output instead of routing around it.
...
| Bound a durable control — a resurrection lock, a tombstone comparison — to the *active* key of a versioned ring, so the next key rotation silently disarmed it while every test ran under one key | A record that stores a key id and version is compared under exactly that key of the ring, and a record whose key the ring no longer holds is refused by name instead of compared under the wrong key. Prove the control once across a rotation before a `TC-` cites it (§6, §7). |
| Seeded a browser smoke in the test process and read the result from a separately started server process, so every configuration the seeding changed — a temporary artifact root — was invisible to the page under test and the filled area never rendered | A smoke that starts its own server passes every seeding-time configuration that page reads through the server's explicit environment, and waits for the filled content — a real finding id, a real path — rather than for the area marker (§4, §7). |
| Keyed a one-way guard transition on the *previous content* of the row (`OLD.output <> marker`) instead of its persisted state, so a row whose regular untrusted content equalled the marker could never take the transition and the sweep aborted on it | A transition guard reads state columns — `deleted_at IS NULL` → `IS NOT NULL` — never the untrusted content the transition replaces. Prove the transition once with content that equals the marker (§6, §7). |
| Persisted a deadline on every new row but left the rows that predate the contract to a read-time derivation from the current configuration, so raising the value showed their expired raw text again and moved their deletion | A migration that introduces a persisted binding binds the existing rows once, under the trusted configuration of that upgrade, in the same migration; a row without the binding afterwards is closed — hidden, named, never derived. Prove it with a rollback, a legacy row and a raised value (§6, §7). |
| Let a model creation hook skip the server binding when the caller had prefilled the bound fields, so any caller could extend a row's retention or forge its tombstone provenance | A server-owned binding is written unconditionally in the hook — expiry, size, fingerprint and creation time from the server, whatever the caller passed — and a forged far-away value is tested once (§6, §7). |
| Signed new identities under the oldest key of a versioned ring so that two processes with different active keys would agree, which made every rotation inert and used a retired key for fresh HMACs | New fingerprints always use the active key. Agreement between concurrent writers comes from serializing lookup and insert — a transaction that opens with a write on the row set — never from a shared retired key (§6). |
| Verified the PHPStan budget only with a warm result cache, so the documented default command crashed on a fresh checkout | Run the documented analysis command once with the result cache removed before reporting it green; a run that dies at the memory limit is not a result (§4). |
| Bound an AC to a test method, let a reflection guard confirm the method exists, and read that green guard as coverage; the method never asserted what the criterion demands | A method binding is an inventory, not evidence. A criterion whose proof does not exist yet belongs in a declared, blocking gap list that the release command prints and that keeps its exit code non-zero — never in a coverage table that a passing suite makes look closed (§7 item 4). |
| Made an unsatisfiable AC satisfiable by narrowing it, or left it open forever, when the blueprint sentence it derives from was the milder one — so meeting the ticket text literally would have deleted the negative tests whose whole subject is the refused write | Check the AC against its blueprint sentence in plan §15 before either move: the plan wins (§2). Where the ticket is stricter than the plan, precise it back toward the blueprint wording under an explicit human release, name the permitted exceptions per scenario, and have the guard assert exactly that list (§10). |
| Treated a version string plus the presence of a flag as proof that an external tool honours a boundary, and wrote the unprovable rest into a code comment — while the tool itself could have been asked | Ask the tool. A pinned binary that can enumerate its own effective feature state, its model catalog or its resolved prompt input gives version-bound, falsifiable evidence without doing any real work; run that probe and refuse the start on drift. Where the tool cannot answer, the fact belongs in a named, still-open gate, never in a comment that reads as verified (§6, §7). |
| Read a green capability probe as proof that the boundary holds *here*. What a binary reports about itself is version evidence: which switches it honours, which models it knows. Whether its sandbox, its network boundary or its isolation actually hold in this container is a different question, and a probe that answers the first says nothing about the second | Separate the two before relying on either. When no turn-free instrument answers the runtime question — check that specifically: the vendor's own sandbox subcommand may need a configuration surface the real call never uses — the fact is asserted separately as trusted instance configuration, bound to values the machine can still check (the pin, the running platform), refused when absent, and sourced from the named gate that observed it. Never let a green version probe stand in for it (§6, §7). |
| Put a cheap textual pre-filter in front of a decoder, so the fast path and the decision path read different values. The raw bytes of an encoded document do not contain what decoding produces — a JSON string escape, a normalized form, a percent-encoding — and an adversary picks exactly that difference | Decide on the decoded value alone. A pre-filter over raw bytes is only safe where a miss is harmless; in front of a security or contract decision it *is* the bypass. Write the escaped variant as a test and watch it fail before the fix (§6, §7). |
| Threw away, on the error path, a fact the tool had already reported and the success path would have recorded — usage, a version, a bound identifier — because the exception carried only a reason | An error is not a reason to lose evidence that already exists. Carry the reported fact on the typed failure and let the consumer persist it, while everything the failure invalidates — the answer bytes, the import — stays behind. Assert the preserved value in the failure test, not only the failure (§7). |
| Repaired that carry on one of two execution paths. The same seam runs in-process here and over the mailbox there, the tests exercised the one the production role uses, and the other path kept dropping the fact — so the criterion held for one runtime and silently failed for the other | A seam with two execution paths has two error paths. Enumerate them before calling such a fix complete — a role switch, an acknowledged reduced mode, an in-process fallback — and give each its own test in the mode that reaches it. Where both paths must produce the same record, name the one writer they share and check the second path against the first, not against the happy case (§5, §7). |
| Left an extension surface unbounded because every known switch was turned off, so the next version's default-on feature would have arrived unnoticed | Pin the *permitted* set, not only the forbidden one: compare the tool's reported effective surface against a version-bound allowlist and refuse anything it reports as active that the approved profile does not name. A deny list only covers what existed when it was written (§6). |
| Passed input that a limit permits as a single command-line argument | Check the argument against the OS limit first — Linux caps one argument at `MAX_ARG_STRLEN` (128 KiB), far below several configured maxima here — and use standard input or a file when it can exceed that. Prove the configured maximum and one byte above it end to end (§4, §7). |
| Let an optional environment value flow unfiltered into a strict configuration list, so an empty value turned one provider's "not set up" state into a fatal error for a shared policy and took an unrelated component down with it | An unset optional value produces no list entry at all. Where a config list mixes a mandatory entry with an optional one, filter the empties at the source and prove once that the empty case locks only its own feature while the shared seam stays usable (§6). |
| Ran the complete regular suite after a review step or a finding fix, because the change touched a shared seam and "everything" felt safer. It is not safer, it is an hour per iteration, and §4 already rules it out: the full run is the completion gate of an implementation ticket, not of a fix iteration | Run exactly what §4 names for an iteration: the new and changed tests, plus the unchanged regression, inventory and architecture tests that pin the contracts this change touches. A shared seam is not a licence to run everything — it tells you *which* consumers to name and run. The full suite belongs to reporting an implementation ticket as done, and to nothing else (§4). |
